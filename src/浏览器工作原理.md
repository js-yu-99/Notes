...
![img](https://cdn.nlark.com/yuque/0/2022/png/21510703/1641480557054-badf7444-1e13-481a-ad4f-7d304a111ad6.png)
...
![img](https://cdn.nlark.com/yuque/0/2022/png/21510703/1641480929311-8c162b4d-6189-4b15-a291-b5af625ec8a0.png)
...
HTTP/1.0 每进行一次 HTTP 通信，都需要经历建立 TCP 连接、传输 HTTP 数据和断开 TCP 连接三个阶段。随着浏览器普及，单个页面中的图片文件越来越多，有时候一个页面可能包含了几百个外部引用的资源文件，如果在下载每个文件的时候，都需要经历建立 TCP 连接、传输数据和断开连接这样的步骤，无疑会增加大量无谓的开销。

![img](https://cdn.nlark.com/yuque/0/2022/png/21510703/1641481466459-74326490-cbce-428b-beb7-2f7ffc744f12.png)

HTTP/1.1 中增加了持久连接的方法，它的特点是在一个 TCP 连接上可以传输多个 HTTP 请求，只要浏览器或者服务器没有明确断开连接，那么该 TCP 连接会一直保持。

持久连接在 HTTP/1.1 中是默认开启的，所以需要专门为了持久连接去 HTTP 请求头设置信息，如果不想要采用持久连接，可以在 HTTP 请求头中加上Connection: close。目前浏览器中对于同一个域名，默认允许同时建立 6 个 TCP 持久连接。

![img](https://cdn.nlark.com/yuque/0/2022/png/21510703/1641481475197-30657fb7-4ab2-4ca2-bb4a-ae3a9a1c5060.png)
...
HTTP/1.1对带宽的利用率并不理想，这是HTTP/1.1的一个核心问题。
...
当同时建立多条TCP连接时，在带宽不足时，这些TCP连接就会减慢发送或者接收的速度。这样就会出现一个问题，因为有的 TCP 连接下载的是一些关键资源，如 CSS 文件、JavaScript 文件等，而有的 TCP 连接下载的是图片、视频等普通的资源文件，但是多条 TCP 连接之间又不能协商让哪些关键资源优先下载，这样就有可能影响那些关键资源的下载速度了。
...
关于队头阻塞的问题，HTTP/2的解决方案是：一个域名只使用一个 TCP 长连接和消除队头阻塞问题。

![img](https://cdn.nlark.com/yuque/0/2022/png/21510703/1641564345209-f735856a-a6bb-4df5-8d4d-37e19dbfc8c4.png)

这就是HTTP/2最重要的特性：多路复用机制。
...
![img](https://cdn.nlark.com/yuque/0/2022/png/21510703/1641564441137-85c18faf-b6b7-4844-a5ba-4ab36fd85ad2.png)
...
浏览器根据资源类型确认优先级，通过优先级告诉服务器该优先处理哪些数据。

优先级大致是 HTML > CSS > Blocking Script > Font >= Image >= Async Script。

可以在浏览器控制台-网络-优先级中查看请求的优先级。



### 2.服务器推送

HTTP/2可以直接将数据提前推送到浏览器。当用户请求一个 HTML 页面之后，服务器知道该 HTML 页面会引用几个重要的 JavaScript 文件和 CSS 文件，那么在接收到 HTML 请求之后，附带将要使用的 CSS 文件和 JavaScript 文件一并发送给浏览器，这样当浏览器解析完 HTML 文件之后，就能直接拿到需要的 CSS 文件和 JavaScript 文件，这对首次打开页面的速度起到了至关重要的作用。
...
![img](https://cdn.nlark.com/yuque/0/2022/png/21510703/1641639805312-8ed38082-43af-46a9-826b-a5ca6a2601e4.png)



TCP传输数据时，从一端发送给另外一端的数据会被拆分为一个个按照顺序排列的数据包，这些数据包通过网络传输到了接收端，接收端再按照顺序将这些数据包组合成原始数据，这样就完成了数据传输。
...
![img](https://cdn.nlark.com/yuque/0/2022/png/21510703/1641639918023-e32ccb8b-0ef1-4cfa-8888-4781f23125c3.png)
...
![img](https://cdn.nlark.com/yuque/0/2022/png/21510703/1641640214407-99ee97d5-6cfe-4189-9227-1c1684186573.png)
...
TCP协议改进起来非常困难。主要是因为中间设备的僵化，中间设备是在互联网各处搭建的各种设备。这些设备包括了路由器、防火墙、NAT、交换机等。它们通常依赖一些很少升级的软件，这些软件使用了大量的 TCP 特性，这些功能被设置之后就很少更新了。
...
基于 UDP 实现了类似于 TCP 的多路数据流、传输可靠性等功能的协议。

![img](https://cdn.nlark.com/yuque/0/2022/png/21510703/1641640452245-5688ce99-7644-4f41-b139-5d304405c8a8.png)
...
![img](https://cdn.nlark.com/yuque/0/2022/png/21510703/1641640526453-f39da549-15d8-46b3-8920-e29e63be62d8.png)
...
从一个页面点击打开另一个页面连接，新标签页中的window.opener的值就是指向第一个标签页中的window，这样就可以在新的标签页中通过opener来操作上一个标签页了。所以说这两个标签页室友连接的。



另外一种方法是通过JavaScript中的window.open 方法来和新标签页建立连接。

```javascript
new_window = window.open('http://www.baidu.com');
```

返回值new_window 是打开的新窗口对象的引用。如果调用失败，返回值会是null。这样可以在当前标签页中通过new_window来控制新标签页，还可以在新标签页中通过window.opener来控制当前标签页。



通过这些方式打开的新标签页，不论这两个标签页是否属于同一站点，它们之间都能通过opener来建立连接，所以它们之间是有联系的。在WhatWG规范中，把这一类具有相互连接关系的标签页称为浏览器上下文组。

通常情况下，把一个标签页所包含的内容，诸如 window 对象，历史记录，滚动条位置等信息称为浏览上下文。这些通过脚本相互连接起来的浏览上下文就是浏览上下文组。

Chrome 浏览器会将浏览上下文组中属于同一站点的标签分配到同一个渲染进程中，这是因为如果一组标签页，既在同一个浏览上下文组中，又属于同一站点，那么它们可能需要在对方的标签页中执行脚本。因此，它们必须运行在同一渲染进程中。



![img](https://cdn.nlark.com/yuque/0/2022/png/21510703/1641737227151-93a62bc5-6452-4c95-8485-49c397aea2fa.png)



a标签的rel属性值设置为noopener，可以让被链接的标签页和当前标签页不要产生连接关系。
...
![img](https://cdn.nlark.com/yuque/0/2022/png/21510703/1641737400560-f2aace52-2ce1-4284-b71d-2992a563cc7d.png)
